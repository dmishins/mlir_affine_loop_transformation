...
struct AffineLoopTilingBtr : public AffineLoopTilingBtrBase<AffineLoopTilingBtr> {
  AffineLoopTilingBtr() = default;
  explicit AffineLoopTilingBtr(uint64_t cacheSizeBytes, bool avoidMaxMinBounds = true)
...
void AffineLoopTilingBtr::getTileSizes(ArrayRef<AffineForOp> band,
                              SmallVectorImpl<unsigned> *tileSizes) {
  if (band.empty())
    return;
  ...
  if (!fp) {
    ...
    std::fill(tileSizes->begin(), tileSizes->end(),
              AffineLoopTilingBtr::kDefaultTileSize);
...
void AffineLoopTilingBtr::runOnOperation() {
...
constexpr unsigned AffineLoopTilingBtr::kDefaultTileSize;
